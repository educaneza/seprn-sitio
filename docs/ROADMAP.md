...
16. **Extender el "planchado" (dropdowns + semáforo + protección de solo aviso) y el manual
    visual a Soporte, Correo y Formación Docente** (piloto en Mantenimiento 1 sep 2026, repetido
    en Asesorías la misma sesión sin sorpresas — ver `docs/ARCHITECTURE.md §24` y
    `docs/manual-bases-tramites.html`): repetir el mismo patrón ya probado dos veces —
    ...
    documentado). Ojo con Correo: `Estado general` es texto libre a propósito (decisión ya tomada
    de no cerrarlo a un catálogo), no convertirlo en dropdown. Ojo también con el diálogo de
    `SpreadsheetApp.getUi().alert()` final: aparece en la pestaña del Sheet, no en la del editor
    de Apps Script — hay que cerrarlo ahí para que el Registro de ejecución marque "completado".
