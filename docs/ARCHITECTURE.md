...
- **Validez oficial USICAMM/PROEEB (sep 2026)**: `Valida_USICAMM`/`Valida_PROEEB` son dos banderas TRUE/FALSE independientes — un curso puede tener una, otra, ambas o ninguna. Se muestran como una sola etiqueta combinada (`.cc-valida-badge`, esquina superior derecha de la tarjeta, espejo de `.cc-sel-badge`): "USICAMM", "PROEEB" o "USICAMM · PROEEB" si ambas aplican. A diferencia de `.cc-sel-badge`, es siempre visible cuando aplica (no depende de si la tarjeta está seleccionada), y se conserva también en las tarjetas del historial — sigue siendo información útil como referencia pasada.
